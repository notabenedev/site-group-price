<div class="price__group-item price__group-item_level-{{ $level }}"  id="{{ $item["slug"]}}">
    <h3 class="h{{$level+1}} price__group-title">{{ $item["title"] }}</h3>
    @isset($item['short'])
    ...
    @foreach(\Notabenedev\SiteGroupPrice\Facades\PriceActions::getGroupPrice($item["id"]) as $id => $price)
        <div class="row price__list {{ ($loop->iteration % 2 == 0) ? "" : "price__list-iteration" }}">
            <div class="col-12 col-sm-7 col-md-8 col-lg-9" id="{{ $price["slug"]}}">
                {{ $price["title"] }}
                @isset($price["description"])
                    {!! $price["description"]  !!}
                @endisset
            </div>
            <div class="col-12 col-sm-5 col-md-4 col-lg-3 font-weight-bold text-secondary text-sm-right">
                @if($price["price"]  )
                    {{ $price["price"] }}
                @endif
            </div>
